...
html"""
<p style="font-size:20px;">#wengineers: Jazmin Rodriguez, Melenie Mendez-Rangel, Shelyn Castellanos, Tasmin Sangha</br>
...
md"""
###### __Assumptions__
All streams can be assumed to be ideal, at atmospheric pressure and isothermal.
"""
...
# ╔═╡ 33b58e33-b950-41f5-b9d2-d97b6abb0a4c
begin
...
# ╔═╡ bea65f85-8def-4935-9764-6f43cb6e8023
md"""
### Results and Discussion
"""

# ╔═╡ 8bfa6a75-cd24-48f0-ad1b-c70dd9631ec4
md"""
##### Compute the number of chips in a series needed to reach 95% purity and a flow rate of 1.0 g/hr
"""

# ╔═╡ 0806c6f5-3eb0-40bf-9513-134d5e1a8187
md"""
...
# ╔═╡ 288ce73d-2724-4bea-bc95-eee7b1002e29
begin

	# setup calculation for chips i = 2,....,N
	N = 97 # number of chips
...
# ╔═╡ 17147951-f9e0-4d22-a5b1-c9edf50e82fe
exit_flag_array

# ╔═╡ 77531a4a-5609-496a-bc26-c100953f016c
status_flag_array

# ╔═╡ 1cde6f7e-4ef4-441f-838a-5f10cefe4260
md"""
##### Table 3: State table describing the exit composition (mol/hr) for each chip
"""

# ╔═╡ f69ab27e-fef5-47b6-97df-b88c77d4f891
begin
...
# ╔═╡ bd513b1f-0af5-40f8-a498-d89b2be5ae8e
md"""
##### Theory of downstream separation using Magical Sepration Units (MSUs)
To separate the desired product from the unreacted starting materials and by-products, let's suppose the teaching team invented a magical separation unit or MSU. MSUs have one stream in, and two streams out (called the top, and bottom, respectively) and a fixed separation ratio for all products (that's what makes them magical), where the desired product is _always_ in the top stream at some ratio $\theta$. In particular, if we denote $i=\star$ as the index for the desired product (in this case isoprene), then after one pass (stream 1 is the input, stream 2 is the top, and stream 3 is the bottom) we have:
...
# ╔═╡ d91a8cc6-5e36-47c6-88f7-457637379973
# how many levels are we going to have in the separation tree?
number_of_levels = 6

# ╔═╡ 2d6d58d5-c0eb-43d3-a6ba-d320b40e3575
begin
...
# ╔═╡ b19efe18-4d6e-401b-aa0c-ca4b9abce4b5
begin
...
# ╔═╡ 5461747f-fa3d-4f6a-abf9-7b1d305f6dde
with_terminal() do
...
# ╔═╡ a09710be-d7ba-43bb-bd50-0f8c51ca670e
md"""
##### Cost Analysis
"""

# ╔═╡ f47d87b5-4d2d-416b-980b-8545bf1337ce
md"""
###### Capital Expenditure
| Items      | Quantity | Cost per unit     | Total|
|    :----:   |    :----:   |    :----:     | :----:|
| Reactor Chips      | 97       | 100   | (9,700) |
| Syringe Pumps   | 1        |  1,000    |  (1,000) | 
|Separator Units| 6| 20 | (120)|

__Total:__     (10,820)
"""

# ╔═╡ fed188db-9b85-41a8-b2f8-3840bf14cf32
md"""
###### Operating Cost
| Chemical      | Price per mmol | mmol/hr    | Hours | Total |
|    :----:   |    :----:   |    :----:     | :----:|  :----:|
| Maltose     | 0.0756651       | 120.834   | 8760 | (80,091.95)|
"""

# ╔═╡ e96e51a8-8e06-4e36-8ab6-c2dbe9709bbf
md"""
###### Profit
| Product      | Price per gram | gram/hr    | Hours | Total |
|    :----:   |    :----:   |    :----:     | :----:|  :----:|
| Maltose     | 27.5805       | 1.00388   | 8760 | 242542.61|
"""

# ╔═╡ 97bfe50a-9968-4069-ad67-c5f05f31fa28
md"""
__After the first year:__  $151,630.66
__Afterwards:__ $162,450.66 
"""

# ╔═╡ f746fc8f-6340-4712-b4d1-8d820bdf0fe0
md"""
##### Is the process profitable when compared to an alternative investment yielding a minimum of 1% per year and a maximum of 10% per year?
"""

# ╔═╡ 306ca98a-5ed2-4e2f-88bd-92692473bd8e
# lifetime = 3 future years (period of time to payback the investment)
T = 10

# ╔═╡ 5fc5cf95-6cc1-4961-8f95-da08cfc31db4
# discount rate: risk-free interest rate
# assume constant over the payback lifetime
discount_rate = 0.10

# ╔═╡ 3415fb3e-95ab-4f42-8b15-25f831756b92
# Setup the cash flow array -
CF_array = [151.6, 162.5, 162.5, 162.5, 162.5, 162.5, 162.5, 162.5, 162.5, 162.5, 162.5]

# ╔═╡ 8cf8b55e-670e-4eaf-8209-95ada83d4447
...
# ╔═╡ 8e534280-bcd0-41fb-af20-521569455f7f
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ 48f3a910-0840-4658-9580-ce1f6f1752d5
# what is the NPV?
NPV = sum(PV)

# ╔═╡ 0d5562a9-6310-40f7-969a-2ba128f357cc
md"""
##### Yes
"""

# ╔═╡ d3579592-e67d-4fbf-95a9-cc0b0a0ea74c
md"""
#### Alternatives ideas
If we had more time, we would have explored more cost efficient methods such as adding different reactants to optimize the flow rate and optimized how much maltose to add to each stream in order to lower the cost. Furthermore, we would have explored how to calculate the flow rate in the middle MSUs, which would allow us to use less reactor chips. Finally, we would have taken into account the byproducts and refined and sold them.
"""
...
# ╔═╡ dfc0cd81-8ea5-4fa1-a59f-9a6fb277b350
md"""
###### [Cost of maltose](https://www.sigmaaldrich.com/US/en/search/maltose?focus=products&page=1&perPage=30&sort=relevance&term=maltose&type=product)
	1 kg for $210.00	
	Product number:M5885
	SKU:M5885-1KG.
[Cost of Isoprene](https://www.sigmaaldrich.com/US/en/search/isoprene?focus=products&page=1&perPage=30&sort=relevance&term=isoprene&type=product)
	1 mL for $40.50
